...
public class HistoricoController {
    ...
    public Map<String, Object> calcularCoeficienteRendimento(int alunoId) {
        ...
        try {
            ...
            response.put("status", "success");
            response.put("coeficiente", coeficiente);
        } catch (SQLException e) {
            response.put("status", "error");
            response.put("message", "Erro ao acessar o banco de dados: " + e.getMessage());
        }
    ...
    private Map<String, Object> converterHistoricoParaMap(Map<Disciplina, Double> historico) {
        ...
        for (Map.Entry<Disciplina, Double> entry : historico.entrySet()) {
            ...
            disciplinaMap.put("nota", entry.getValue());
            
            result.put("disciplina_" + disciplina.getId(), disciplinaMap);
        }
        return result;
    }
}
